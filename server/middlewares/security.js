...
if (process.env.SAFE_ENV === 'safe') {
  securityMiddleware = [
    ...securityMiddleware,
    // Las rutas se protejen individualmente por ruta dentro de API
    xoxo,
    csrf({ cookie: true }),
    ...
    (err, req, res, next) => {
    // Tenemos que revisar si una primera solicitud proviene
    // desde node.js y no desde el navegador, si es asi, se
    // Valida el x-xoxo para revisar que la solicitud provenga
    // de el servidor genuinamente
      if (req.checkXoxoToken(req)) {
        return next()
      }
