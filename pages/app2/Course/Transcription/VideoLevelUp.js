...
const Button = styled.div`
  ...
  opacity: 1;
  border: none;
  background-color: ${p =>p.color || '#0273D4'};
  padding: 1.3em 3em;
  border-radius: 9px;
...
const Play = ({show, onClick, color}) => (
  <Box show={show} onClick={onClick}>
    <Button color={color}>
      <Icon type='play' margin='0' size='50px' color='#FFF' />
    </Button>
  </Box>
)
  ...
  render () {
    return (
      <UpWraper>
        ...
        <Play show onClick={this.play} color={this.props.color} />
      </UpWraper>
    )
  }
}
